...
| **D2 + D3** | Onde mora UI engenheiro (live + post)? | **✅ FECHADA 2026-05-13** _(mecanismo atualizado 16/06/2026 — ver `docs/ARQUITETURA_DEFINITIVA.md`)_ | **Command Box Engenharia = TV 32" via Fire TV Stick 4K Max abrindo o app na nuvem no navegador** (engenheiro + chefe-de-equipe) — substitui o iOS Box Mode → AirPlay → Apple TV decidido em 2026-05-13. **Cockpit Pilot = notebook WinUI 3 → display 2 10,5" invertida** (piloto, igual MS-13). **Operação pelos celulares acessando o app na nuvem** — notebook não é operado, fica kiosk. Quando carro parado, Cockpit Pilot ganha **overlay de simulação** sincronizado por Realtime com o que o engenheiro está propondo na TV 32" (sem display switching). |
