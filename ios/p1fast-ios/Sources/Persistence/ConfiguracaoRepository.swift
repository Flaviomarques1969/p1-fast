...
final class ConfiguracaoRepository: ObservableObject {
    ...
    func bootstrap() async {
        do {
            try await reload()
            try await semearSetupCeltaBubiSeNecessario()
        } catch {
            print("ConfiguracaoRepository.bootstrap failed: \(error)")
        }
    }

    /// Aplica o catálogo Celta Bubi (Flávio confirmou 2026-05-18) nas
    /// configurações do carro chamado "Celta", "Bubi" ou "Bolinha".
    /// Idempotente — só preenche se o setup técnico ainda estiver vazio.
    /// Roda no bootstrap pra que a 1ª abertura após v33 já tenha cadastro
    /// pronto sem o gestor precisar digitar.
    private func semearSetupCeltaBubiSeNecessario() async throws {
        let candidatas = configuracoes.filter { !$0.temSetupTecnicoBasico }
        guard !candidatas.isEmpty else { return }
        let carrosPorId: [String: Carro] = try await queue.read { db in
            let ids = candidatas.map { $0.carroId }
            let carros = try Carro.filter(ids.contains(Column("id"))).fetchAll(db)
            return Dictionary(uniqueKeysWithValues: carros.map { ($0.id, $0) })
        }
        for cfg in candidatas {
            guard let carro = carrosPorId[cfg.carroId] else { continue }
            let apelidoLower = carro.apelido.lowercased()
            let bate = apelidoLower.contains("celta")
                || apelidoLower.contains("bubi")
                || apelidoLower.contains("bolinha")
            guard bate else { continue }
            try await semearCeltaBubiSeVazio(cfg)
        }
    }
    ...
    /// Atualiza apenas os campos do setup técnico (v33 — Bloco 1 do plano
    /// Piloto + Engenheiro 2026-05-18). Não toca em nome/motor/pneu/aero —
    /// esses ficam no atualizar(...) padrão. Cadastro incremental: aceita
    /// nil em qualquer campo (o gestor pode preencher só o que sabe agora).
    func atualizarSetupTecnico(_ cfg: Configuracao,
                                relacaoMarcha1: Double?,
                                relacaoMarcha2: Double?,
                                relacaoMarcha3: Double?,
                                relacaoMarcha4: Double?,
                                relacaoMarcha5: Double?,
                                relacaoMarcha6: Double?,
                                relacaoFinal: Double?,
                                raioPneuM: Double?,
                                massaCarroKg: Double?) async throws {
        var copia = cfg
        copia.relacaoMarcha1 = relacaoMarcha1
        copia.relacaoMarcha2 = relacaoMarcha2
        copia.relacaoMarcha3 = relacaoMarcha3
        copia.relacaoMarcha4 = relacaoMarcha4
        copia.relacaoMarcha5 = relacaoMarcha5
        copia.relacaoMarcha6 = relacaoMarcha6
        copia.relacaoFinal = relacaoFinal
        copia.raioPneuM = raioPneuM
        copia.massaCarroKg = massaCarroKg
        copia.updatedAt = DB.nowMs()
        try await queue.write { db in
            try copia.update(db)
        }
    ...
    /// Aplica os valores iniciais do catálogo Celta Bubi (Flávio confirmou
    /// 2026-05-18). Idempotente — se a configuração já tem setup técnico,
    /// NÃO sobrescreve (preserva ajustes manuais do gestor).
    func semearCeltaBubiSeVazio(_ cfg: Configuracao) async throws {
        guard !cfg.temSetupTecnicoBasico else { return }
        try await atualizarSetupTecnico(
            cfg,
            relacaoMarcha1: CeltaBubiCatalogo.relacaoMarcha1,
            relacaoMarcha2: CeltaBubiCatalogo.relacaoMarcha2,
            relacaoMarcha3: CeltaBubiCatalogo.relacaoMarcha3,
            relacaoMarcha4: CeltaBubiCatalogo.relacaoMarcha4,
            relacaoMarcha5: CeltaBubiCatalogo.relacaoMarcha5,
            relacaoMarcha6: nil,
            relacaoFinal: CeltaBubiCatalogo.relacaoFinal,
            raioPneuM: CeltaBubiCatalogo.raioPneuM,
            massaCarroKg: CeltaBubiCatalogo.massaCarroKg
        )
    }
