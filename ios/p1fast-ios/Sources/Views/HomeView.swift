...
enum HomeNavTarget: Hashable, Codable {
    ...
    case estoque
    case manutencao
    /// 2026-05-18 (Flávio "go" autônomo noturno) — Manutenção POR carro,
    /// acessada pelo painel do carro. Diferente de `.manutencao` (atalho
    /// global da Garagem) que continua sendo placeholder até o painel
    /// "Pronto pra pista" do Bloco 6 entrar.
    case manutencaoCarro(carroId: String)
    /// 2026-05-18 — Bloco 1 do plano Piloto + Engenheiro (Flávio mandou
    /// seguir após confirmar valores). Formulário de setup técnico (relações
    /// de marcha + diferencial + raio do pneu + massa do carro). Habilita
    /// cálculo de aceleração teórica.
    case setupTecnicoCarro(carroId: String)
    ...
    /// 2026-05-18 — Bloco 6 do plano Piloto + Engenheiro. Tela mostrada
    /// após encerrar o stint OU acessível direto (modo exemplo) pelo
    /// painel do carro até o T4000 chegar.
    case debriefStint(carroId: String)
    /// 2026-05-18 — Bloco 6. Linha do tempo dos últimos 5 eventos
    /// por trecho. Entra pela aba Estatísticas do painel do carro.
    case evolucaoCarro(carroId: String)
}
...
struct HomeView: View {
    ...
    private func destinationView(for target: HomeNavTarget) -> some View {
        ...
        case .manutencaoCarro(let carroId):
            ManutencaoCarroView(carroId: carroId, onClose: { navCoordinator.popLast() })
        case .setupTecnicoCarro(let carroId):
            SetupTecnicoView(carroId: carroId, onClose: { navCoordinator.popLast() })
        case .autodromosLista:
        ...
        case .debriefStint(_):
            DebriefView(
                resumo: debriefExemplo(),
                estoque: [
                    DisponibilidadeEstoque(item: "vela NGK iridium", quantidade: 4, localPrincipal: "Box"),
                    DisponibilidadeEstoque(item: "filtro de ar K&N", quantidade: 1, localPrincipal: "Caminhão"),
                ],
                onClose: { navCoordinator.popLast() }
            )
        case .evolucaoCarro(let carroId):
            EvolucaoCarroView(carroId: carroId, onClose: { navCoordinator.popLast() })
        }
    }
