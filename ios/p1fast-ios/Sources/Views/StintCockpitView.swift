...
struct StintCockpitView: View {
    ...
    init(
        ...
        let ajuste = CockpitAjusteUD.carregar()
        _escalaExtra = State(initialValue: ajuste.escala)
        _offsetX = State(initialValue: ajuste.dx)
        _offsetY = State(initialValue: ajuste.dy)
    }
    ...
    var body: some View {
        ...
        .overlay(alignment: .topTrailing) {
            // Zona invisível 80×80 no canto superior direito. Toque triplo
            // alterna o modo engenharia (ajuste + encerrar).
            Color.clear
                .frame(width: 80, height: 80)
                .contentShape(Rectangle())
                .onTapGesture(count: 3) {
                    mostrarEngenharia.toggle()
                }
        }
...
private struct CockpitWebView: UIViewRepresentable {
    ...
    (function(){
      ...
      // ResizeObserver reage a qualquer mudança de tamanho (inclui o
      // momento em que o WebView finalmente reporta dimensão real).
      if(window.ResizeObserver){
        try {
          var ro = new ResizeObserver(function(){ aplicarTransform(); });
