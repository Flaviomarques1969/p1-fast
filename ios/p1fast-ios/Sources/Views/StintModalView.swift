...
struct StintModalView: View {
    ...
    // Onda 2 (2026-05-24): seletor de carro obrigatório (antes era inferido)
    // e toggle de IA padrão LIGADA — piloto pode desligar em sessão de
    // aquecimento/shake-down.
    @State private var carroId: String?
    @State private var iaLigada: Bool = true
    ...
    var body: some View {
        ...
        .onAppear {
            hidratarPilotoDefault()
            hidratarCarroDefault()
        }
    ...
    // Onda 2: carroIdInferido mantido pra usos internos (pneuPicker, etc.)
    // mas a sessão usa `carroId` selecionado explicitamente pelo piloto.
    private var carroIdInferido: String? {
        carroId ?? carroRepo.carros.first?.id
    }

    private var canSave: Bool {
        guard !isSaving else { return false }
        guard pilotoId != nil else { return false }
        guard carroId != nil else { return false }
    ...
    private var sectionConfiguracao: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            // Onda 2: seletor de carro só aparece quando há mais de 1 carro.
            // Com 1 carro cadastrado, o app pré-seleciona e oculta o picker —
            // não tem sentido escolher entre 1 opção.
            if carroRepo.carros.count > 1 {
                FormField(label: "Carro", small: "obrigatório") { carroPicker }
            }
            FormField(label: "Análise da IA", small: "ligada por padrão") {
                iaToggle
            }
    ...
    private var carroPicker: some View {
        Menu {
            ...
            HStack {
                Text(carroNomeAtual ?? "Escolher carro")
                    .font(.system(size: 15, weight: .medium))
                    .tracking(-0.075)
                    .foregroundStyle(carroNomeAtual == nil ? Color.textFaint : Color.text)
                Spacer()
                Text("›")
    ...
    private var carroNomeAtual: String? {
        guard let id = carroId else { return nil }
        return carroRepo.carros.first(where: { $0.id == id })?.apelido
    }

    private func hidratarCarroDefault() {
        if carroId == nil {
            carroId = carroRepo.carros.first?.id
        }
    }

    // MARK: - Toggle IA (Onda 2)

    private var iaToggle: some View {
        HStack {
            Toggle("", isOn: $iaLigada)
                .labelsHidden()
                .tint(Color.accentColor)
            Spacer()
            Text(iaLigada ? "IA analisará este stint ao vivo" : "Sem análise da IA neste stint")
                .font(.system(size: 13, weight: .regular))
                .foregroundStyle(iaLigada ? Color.text : Color.textMuted)
        }
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
        .background(
    ...
    private func salvar() {
        guard let pid = pilotoId, let cid = carroId else { return }
        isSaving = true
        savingError = nil
