...
enum Migrations {
    static func register(into m: inout DatabaseMigrator) {
        ...
        // v32 — Função Manutenção (Flávio 2026-05-18, comando "go" autônomo
        // noturno). 3 tabelas locais — produção não tocada nesta rodada:
        //   manutencoes              = registro cronológico de cada troca/serviço
        //   manutencao_especificacoes = especificação padrão por carro+item
        //   manutencao_periodicidades = regra "quando trocar" por carro+item
        // ManutencaoItem fica embutido em Swift (catálogo de 15 defaults).
        m.registerMigration("v32_manutencoes") { db in
            try db.execute(sql: """
                CREATE TABLE manutencoes (
                    ...
                    observacao      TEXT,
                    foto_url        TEXT,
                    peca_id         TEXT REFERENCES pecas(id) ON DELETE SET NULL,
            ...
            """)
            try db.execute(sql: "CREATE INDEX idx_manutencoes_carro_item ON manutencoes(carro_id, item_codigo);")
            try db.execute(sql: "CREATE INDEX idx_manutencoes_ocorrido ON manutencoes(ocorrido_em);")
            try db.execute(sql: "CREATE INDEX idx_manutencoes_time ON manutencoes(time_id);")
            ...
            try db.execute(sql: """
                CREATE TABLE manutencao_especificacoes (
                    ...
                    updated_at      INTEGER NOT NULL,
                    synced_at       INTEGER,
                    UNIQUE (time_id, carro_id, item_codigo)
                );
            """)

            try db.execute(sql: """
                CREATE TABLE manutencao_periodicidades (
                    id              TEXT PRIMARY KEY,
                    time_id         TEXT NOT NULL,
                    carro_id        TEXT NOT NULL REFERENCES carros(id) ON DELETE CASCADE,
                    item_codigo     TEXT NOT NULL,
        ...
        // Piloto + Engenheiro). Adiciona 9 colunas opcionais em `configuracoes`
        // pra habilitar cálculo de aceleração teórica (motor + transmissão +
        // chassi). Todas Double, todas opcionais — cadastro incremental.
        m.registerMigration("v33_setup_tecnico") { db in
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_1 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_2 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_3 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_4 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_5 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_marcha_6 REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN relacao_final REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN raio_pneu_m REAL;")
            try db.execute(sql: "ALTER TABLE configuracoes ADD COLUMN massa_carro_kg REAL;")
        }
    }
