# TASK_INIT 13/06/2026 — CLASSIFICADOR DE CURVAS (1ª FATIA: 8 curvas de Brasília offline + página de validação)

1. Pedido original (Flávio, literal): "pode começar o classificador" (após decidir a decisão 3:
   "Sim, liberar a 1a fatia").
2. Objetivo em 1 frase: classificar as 8 curvas de Brasília fora do carro com os dados reais já
   existentes (geometria + telemetria das melhores passagens) e entregar uma página pro Flávio
   validar curva a curva — sem tocar produção nem o cockpit ao vivo.
3. Critérios de conclusão: (a) módulo puro classificador-trecho.js com teste automático verde;
   (b) script offline que roda sobre as 8 curvas reais e gera a classificação (tipo + formato
   proposto + variáveis, cada uma com confiança/fonte); (c) página de validação curva a curva
   aberta por mim; (d) honestidade do dado declarada (telemetria de maio ~1 Hz → raio/perfil
   grosseiros; classificação primária apoia na velocidade Ve/Vmin); (e) coluna `tipo` de
   track_segments NÃO sobrescrita (dicionário tipo_curva próprio); (f) nada em produção/cockpit ao
   vivo; (g) revisão adversarial.
4. Leitura confirmada: proposta auditada v2 lida inteira · PONTOS-TRECHOS-BRASILIA (8 curvas) lido ·
   passagens-bubi-aplicadas.json (56 passagens reais; pontos_json/_curva/tempo_trecho_s) inspecionado ·
   CLAUDE.md projeto + 6 protocolos (mesma sessão) · memória do ditado (rodada 18) lida.
5. Plano ≤5 passos: (1) ambiente isolado novo a partir da oficial; (2) classificador-trecho.js
   (geometria sobre pontos reais + razão Ve/Vmin + contexto + árvore 6 tipos+SF, limiares declarados
   como chute educado) + teste; (3) script classificar-brasilia.mjs sobre as 8 curvas; (4) página de
   validação curva a curva; (5) revisão adversarial + abrir pro Flávio.
6. Arquivos/áreas: _design-reference/PONTOS-TRECHOS-BRASILIA + TRAJETO-LIMPO (geometria) ·
   ~/Documents/p1fast-backup-voltas-reais/passagens-bubi-aplicadas.json (telemetria real, leitura) ·
   módulo + script + página novos · tests/.
7. Ambiente alvo: desenvolvimento (ambiente isolado novo).
8. Produção protegida: sim (banco p1-fast, p1t4000, cockpit ao vivo intocados).
9. Autorização para produção: não.
10. Evidência da autorização para produção: não recebida (não necessária — entrega offline).
11. Riscos: dado de maio ~1 Hz (raio/perfil grosseiros) → declarar confiança por variável,
    classificação primária na velocidade; limiares iniciais arbitrários (declarados, a calibrar com
    as voltas reais); casar passagem→curva pelo nome (_curva).
12. Status: EM EXECUÇÃO 13/06 — montando ambiente + módulo.

---

# TASK_INIT 13/06/2026 — REABRIR PAINEL COCKPIT-TREINO TRAIL PRA VALIDAÇÃO + PÁGINA DE 3 DECISÕES

1. Pedido original (Flávio, literal): "sim. reabra. e deixe onde couber campo para cada item
   para eu escolher a opção e/ou fazer recomendação"
2. Objetivo em 1 frase: reabrir o painel do cockpit-treino trail rodando em tempo real (voltas
   reais, sem gravar nada) pra validação visual + entregar uma página com os 3 itens em aberto,
   cada um com opções selecionáveis e minha recomendação destacada.
3. Critérios de conclusão: (a) painel reaberto no navegador em tempo real, zero erros de página;
   (b) página de decisões aberta com os 3 itens (critério do CERTO · gráfico-alvo · classificador),
   cada um com opções + recomendação + campo livre + exportação de JSON no padrão de cards;
   (c) nada incorporado à versão oficial, produção (banco e p1t4000) intocada; (d) navegador
   aberto por mim.
4. Leitura confirmada: ~/.claude/CLAUDE.md sim · ~/.claude-decisoes/padroes.md sim (vazio, 0
   decisões) · FLAVIO_EXECUTION_PROTOCOL sim · FLAVIO_DONE_CHECKLIST sim · FLAVIO_ENVIRONMENT_RULES
   sim · FLAVIO_COMMUNICATION_RULES sim · CLAUDE.md do projeto sim · registro ultima-tarefa
   (frente cockpit-treino) lido · memória do ditado (20 rodadas) lida.
5. Plano ≤5 passos: (1) registrar este TASK_INIT; (2) reabrir o painel em tempo real
   (node tools/replay-trail-local.mjs 10 manter 1 no ambiente isolado cockpit-treino-trail —
   sobe servidor próprio 8123, blindagem de não-gravação __P1_ORIGEM_SIM__); (3) montar página de
   3 decisões no padrão de cards (opções + recomendação + campo livre, exporta JSON); (4) abrir as
   duas no navegador eu mesmo; (5) aguardar escolhas + validação visual.
6. Arquivos/áreas: tools/replay-trail-local.mjs (leitura) · web/cockpit/index-t3000.html (painel
   já pronto) · .claude-perguntas/pendentes/ (página nova de decisões). Nada de produção.
7. Ambiente alvo: desenvolvimento (ambiente isolado; replay com servidor local 8123).
8. Produção protegida: sim (banco p1-fast e p1t4000.vercel.app intocados).
9. Autorização para produção: não.
10. Evidência da autorização para produção: não recebida (não necessária — nada vai pro ar).
11. Riscos: replay em tempo real é demorado (10 voltas reais) — o painel abre na hora e dá pra
    validar desde a 1ª volta; página de decisões não altera nada sozinha (só registra escolhas).
12. Status: CONCLUÍDO 13/06 ~11h — AGUARDANDO VALIDAÇÃO VISUAL + 3 DECISÕES DO FLÁVIO.
    FEITO: (a) painel reaberto em tempo real — node tools/replay-trail-local.mjs 10 manter 1
    (processo background br0jam1r2; log /tmp/trail-replay-13jun.log). Prova: "servidor local:
    http://localhost:8123" + "painel pronto: coreografia + cockpit-treino TRAIL armados" +
    "volta 0 · fase fora". Chromium aberto pelo Playwright (headless:false), tempo real (ritmo 1).
    Conflito de porta 8123 resolvido antes (PID 20320 órfão do replay de ontem encerrado).
    Blindagem __P1_ORIGEM_SIM__ ativa = nada grava. (b) Página de 3 decisões criada e aberta:
    .claude-perguntas/pendentes/20260613-p1fast-decisoes-cockpit-treino.html (grade 3 colunas,
    opções + recomendação pré-marcada + campo Outra + exporta JSON no padrão de cards, sem
    emojis, tratamento você). Recomendações: (1) trail certo = 3 de 3; (2) gráfico-alvo em toda
    reta; (3) liberar 1ª fatia do classificador. Produção (banco p1-fast + p1t4000) INTOCADA.
    Nada incorporado à versão oficial. Pendências (ações do Flávio, não minhas): validar o
    painel + devolver as 3 decisões (exportar JSON e dizer "fiz", ou responder no chat).
...
12. Status: EM EXECUÇÃO 12/06 ~13h — construção + 2 rodadas de validação feitas.
    FEITO (ambiente isolado p1fast-worktrees/cockpit-treino-trail, linha
    claude/cockpit-treino-trail a partir da oficial):
    - freio-trecho.js + revisao-treino.* incorporados (29 testes ok).
    - MOTOR novo trail-cockpit-motor.js (lógica pura): alvo por trecho da curva
      REAL da melhor passagem, contagem com ZERO ADIANTADO pela reação aprendida
      (EMA por trecho, persistida, com blindagem anti-simulador), colunas de luz
      (régua 130/105/80/55/35/18 m), pintura verde/vermelho na banda ±8%,
      veredito 3-de-3 com erro nomeado, trilho última+anterior, calibração
      2 passagens. 36 testes ok.
    - TELA nova trail-cockpit-tela.js: criada SÓ com treino armado (sem treino =
      painel intocado), componentes reais (LED/apex), clarão clonado (não briga
      com o renderizador), pisca de erro 3 pulsos + brasa, trilho com DOM
      persistente (célula 'agora' pulsa de verdade), marcha sem onda no modo
      treino, fundo sempre preto.
    - RELIGAÇÃO em main-t3000.js (4 caminhos de armação; orientação cede pro
      gráfico; resumo da volta preservado; régua nasce na pista via
      passagem-salva → atualizarReferencia). 9 testes de religação ok.
    - TELA 10,5 (cobrança Flávio 12/06): palco 956×590 (~3:2), sem recorte de
      celular, sem moldura, preenchendo a tela — só na página do painel ao vivo
      (cockpit.css canônico intocado, teste byte a byte preservado). MEMÓRIA
      gravada: feedback-p1fast-painel-tela-105-nao-celular.
    - HONESTIDADE DO DADO: melhores do banco têm 4-19 pontos/trecho (1 Hz de
      maio) — régua de freada seria INVENTADA. Trava de densidade (≥15 pontos,
      ≤8 m entre eles) + pico mínimo 2,5 m/s²: referência ruim = trecho
      calibrando, sem alvo. Alvo nasce na pista com as passagens densas reais.
    - REVISÃO ADVERSARIAL (53 agentes): 36 achados confirmados, TODOS corrigidos
      (blindagem da reação, clarão no freou-cedo, residual pós-ápice, veredito
      com fim, falso-positivo do pisca, viés da reação, janela órfã, passagem
      rala sem juízo, transição canônica preservada, rótulo FÍSICA GPS no selo,
      package.json restaurado, ferramenta de replay com trava de não-gravação).
      11 falsos alarmes derrubados. Limitação declarada: camada visual sem teste
      de unidade próprio (coberta pelo replay real no navegador, zero erros).
    - Bateria completa do projeto: VERDE (saída 0) após cada rodada.
    REPLAY DEFINITIVO 12/06 ~12h40 (formato 10,5): saída 0, zero erros de
    página, 12 ciclos completos (aproximação→zero→pisou→pintura→veredito→painel),
    1º TRAIL CERTO real no trilho (CURVA "S"), PROVA DE NÃO-GRAVAÇÃO como trava
    automática: OK — nada gravado. Fotos: /tmp/trail-replay-{aproxima,freia,veredito}.png.
    HONESTIDADE: nos dados de bancada só a CURVA "S" tem freada ≥2,5 m/s² —
    demais trechos ficam calibrando até passagem real densa (regra, não defeito).
    RODADA 19 DO DITADO (12/06 ~16h, registrada na memória canônica) APLICADA:
    (1) TRILHO POR VOLTA — célula só do que JÁ foi passado na volta em curso,
    à frente livre, virada de volta limpa (setVoltaAtual ligado na chegada;
    substitui a janela última+anterior da rodada 11); (2) TELA INTEIRA verde
    piscando na freada DENTRO da margem / vermelha piscando fora; (3) indicador
    APERTA/SOLTA o freio à direita, sobre os dados de SAÍDA (freio abaixo do
    alvo = APERTA, acima = SOLTA). 38 testes do motor ok + bateria completa
    verde + replay: saída 0, "S: certo" por volta, nada gravado, zero erros.
    Mockup VIVO ficou ATRÁS da implementação a partir da rodada 19 (anotado).
    RODADA 20 (12/06 ~16h30, na memória canônica): (1) demonstração em TEMPO
    REAL (acelerado só pra teste automático — "não dá pra ver nada"); (2)
    CALIBRAÇÃO = 1 VOLTA só na PRIMEIRA vez com a configuração; base guardada
    arma direto (calibrando = só trecho sem base digna; exigência de 2 voltas
    por sessão removida — era invenção minha). 38 testes ok + bateria verde.
    NAVEGADOR REABERTO EM TEMPO REAL 12/06 ~16h40 (10 voltas, comando
    `node tools/replay-trail-local.mjs 10 manter 1`). AGUARDANDO VALIDAÇÃO.
    Produção INTOCADA (nem banco, nem painel no ar). Decisões pendentes dele:
    critério do CERTO 3-de-3 (default aplicado, ajustável) · OK literal pra
    1ª fatia do classificador (fora desta entrega).
