...
create index on public.padroes_telemetria_por_volta (carro_id, track_id, tipo_pneu);
create index on public.padroes_telemetria_por_volta (atualizado_em);
...
create policy padroes_write_team on public.padroes_telemetria_por_volta
  for all using (
    exists (
      ...
      join public.pessoa_papeis pp on pp.time_id = c.time_id
      where c.id = padroes_telemetria_por_volta.carro_id
        and pp.pessoa_id = auth.uid()
    )
  );
...
create index on public.melhores_passagens_trecho (carro_id, track_id, tipo_pneu);
create index on public.melhores_passagens_trecho (segment_id, tempo_trecho_s);
create index on public.melhores_passagens_trecho (gravado_em);
...
INSERT INTO public.track_layouts (id, track_id, nome, origem)
VALUES ('0dc85cfb-6236-567e-814c-eddf610b301f', 'e8335412-3312-54fe-b634-db2d02c7fa81', 'Principal', 'seed-2026-05-28')
...
-- DROP TABLE IF EXISTS public.padroes_telemetria_por_volta CASCADE;
